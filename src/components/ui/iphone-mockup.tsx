...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  model = '15-pro', color = 'natural-titanium', scale = 1, bezel, radius,
  shadow = true, screenBg = '#000', wallpaper, wallpaperFit = 'cover',
  wallpaperPosition = 'center', showDynamicIsland, showNotch,
  ...
  const homeIndicatorStyle: CSSProperties = {
    ...
    width: Math.round(screenWidth * 0.34), maxWidth: 140, height: 5, borderRadius: 3,
    background: 'linear-gradient(180deg, rgba(255,255,255,0.7), rgba(255,255,255,0.35))',
    opacity: 0.9, zIndex: 3, pointerEvents: 'none',
  };

  const contentStyle: CSSProperties = safeArea
    ? { position: 'absolute', top: insets.top, right: insets.right, bottom: insets.bottom, left: insets.left, overflow: 'hidden', zIndex: 1, display: 'flex', flexDirection: 'column' as const }
