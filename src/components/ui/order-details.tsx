...
export const ORDER_ETA_MAP: Record<string, string> = {
  ...
  shipped: "سيتم التوصيل داخل بنغازي خلال يوم عمل واحد، وللمدن الأخرى 3-5 أيام عمل",
...
export function OrderDetails({
  ...
  const sectionHeading = "text-center text-[11px] font-bold tracking-[0.28em]";

  const timelineSection = (
    <>
      <h3 className={`${sectionHeading} mb-6`} style={{ color: "#b48a45" }}>
        مسار الطلب
      </h3>
      <OrderTracking
        className="mx-auto"
  ...
  const piecesSection = pieces.length > 0 && (
    <div className="space-y-4">
      <h3 className={sectionHeading} style={{ color: "#b48a45" }}>
        القطع الموثّقة
      </h3>
      {pieces.map((piece, i) => (
        <AuthenticatedProductCard
  ...
  const paymentBlock = (
    <div className="flex items-start gap-2.5">
      ...
      <div>
        ...
        <p className="mt-2 text-[13px] font-semibold" style={{ color: "#5c5348" }}>
          الدفع عند الاستلام 💵
        </p>
      </div>
    </div>
  ...
  const deliveryBlock = (
    <div className="flex items-start gap-2.5">
      ...
      <div>
        ...
        <p className="mt-2 text-[13px] leading-relaxed" style={{ color: "#5c5348" }}>
  ...
  /* Unified — one main segment containing every section (success ticket). */
  if (unified) {
    return (
      <div
        className={cn("rounded-3xl bg-white p-5", className)}
        style={{
          border: "1px solid rgba(201,162,94,0.35)",
          boxShadow: "0 18px 44px -30px rgba(34,32,28,0.25)",
        }}
      >
        <div className="divide-y divide-[rgba(201,162,94,0.25)]">
          <section className="pb-5">{timelineSection}</section>
          {piecesSection && <section className="py-5">{piecesSection}</section>}
          <section className="py-5">{paymentDeliverySection}</section>
          <section className="pt-5">{docsSection}</section>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      {piecesSection && <div className="space-y-4">{piecesSection}</div>}
