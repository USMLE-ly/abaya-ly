...
export function SlideTabs({ tabs, selectedIndex, onSelect }: SlideTabsProps) {
  ...
  return (
    <ul
      ...
      className="relative mx-auto flex w-fit rounded-full border border-white/10 bg-white/[0.04] backdrop-blur-xl p-1"
    >
      {tabs.map((tab, i) => (
        <Tab
          key={tab}
...
>(({ children, setPosition, onClick }, ref) => {
  return (
    <li
      ...
      className="relative z-10 block cursor-pointer px-4 py-2 text-xs font-medium text-white/50 hover:text-white transition-colors duration-200 md:px-6 md:py-3 md:text-sm"
    >
      {children}
    </li>
  );
...
const Cursor = ({ position }: { position: { left: number; width: number; opacity: number } }) => {
  return (
    <motion.li
      ...
      className="absolute z-0 h-7 rounded-full bg-brand/20 border border-brand/30 md:h-10"
    />
  );
};
