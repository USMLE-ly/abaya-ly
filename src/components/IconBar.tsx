  ...
  return (
    <section className="py-20 md:py-32">
      <div className="mx-auto w-full max-w-6xl space-y-12 px-4">
        ...
        >
          <span className="inline-block text-xs font-semibold tracking-widest text-brand/60 uppercase mb-4">مميزاتنا</span>
          <h2 className="font-display text-3xl md:text-5xl font-bold tracking-tight text-white text-balance leading-tight">
            لماذا <span className="text-brand">تختاريننا</span>؟
          </h2>
          <p className="mt-5 text-sm md:text-base tracking-wide text-white/30 text-balance leading-relaxed max-w-xl mx-auto">
            نقدم لكِ ما لا تجدينه لدى الآخرين — جودة لا تُضاهى بلمسة ليبية أصيلة
          </p>
        </motion.div>

        <div className="grid grid-cols-1 divide-y divide-white/[0.04] border border-white/[0.04] sm:grid-cols-2 md:grid-cols-3">
          {features.map((feature, i) => (
            <FeatureCard key={i} feature={feature} index={i} className="border-white/[0.04]" />
          ))}
        </div>
