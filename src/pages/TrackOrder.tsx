    ...
    <div className="min-h-screen">
      <section className="pt-24 pb-10 md:pt-28 md:pb-14">
        <div className="max-w-[900px] mx-auto px-4 sm:px-6 text-center">
          <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-3">تتبع <span className="text-primary">طلبكِ</span></h1>
          <p className="text-sm text-foreground/50">أدخلي رقم الطلب والبريد الإلكتروني لمتابعة شحنتكِ</p>
        </div>
      </section>
          ...
          {!tracked ? (
            <form onSubmit={handleSubmit} className="glass-card p-6 md:p-8">
              <div className="space-y-5">
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1.5">رقم الطلب</label>
                ...
                <button type="submit" className="w-full py-3 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-colors text-sm">تتبع الطلب</button>
              </div>
            </form>
          ) : (
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 md:p-8">
              <div className="text-center mb-8">
                ...
                <h2 className="text-lg font-semibold text-foreground">طلب رقم {orderNumber}</h2>
                <p className="text-xs text-foreground/40">آخر تحديث: اليوم</p>
              </div>
              <div className="space-y-5">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${step.done ? "bg-green-500/10 border border-green-500/20 text-green-400" : "glass text-foreground/30"}`}><step.icon size={16} /></div>
                    <div className="flex-1">
                      <p className={`text-xs font-medium ${step.done ? "text-foreground" : "text-foreground/30"}`}>{step.label}</p>
                      <p className="text-[10px] text-foreground/30">{step.time}</p>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => setTracked(false)} className="w-full mt-8 py-3 glass text-foreground font-medium rounded-xl hover:bg-black/5 transition-colors text-xs">تتبع طلب آخر</button>
            </motion.div>
          )}
        </div>
      </section>
