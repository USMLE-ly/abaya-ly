    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-[800px] mx-auto px-4 sm:px-6">
        <nav className="flex items-center gap-2 text-xs text-foreground/40 mb-8">
          ...
          <ChevronLeft size={12} />
          <span className="text-foreground font-medium">سياسة الإرجاع والاستبدال</span>
        </nav>

        <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-4">سياسة الإرجاع <span className="text-primary">والاستبدال</span></h1>
        <p className="text-sm text-foreground/50 mb-10 leading-relaxed">رضاؤكِ هو أولويتنا — تعرفي على سياسة الإرجاع والاستبدال الخاصة بنا.</p>
        ...
        <div className="glass-card p-6 rounded-2xl mb-8">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-primary/15 to-primary/5 border border-primary/10 flex items-center justify-center">
              <RotateCcw className="text-primary" size={18} strokeWidth={1.5} />
            </div>
            <h2 className="font-display text-lg font-bold text-foreground">سياسة الإرجاع</h2>
          </div>
          <p className="text-xs text-foreground/50 leading-relaxed mb-4">يمكنكِ إرجاع المنتج خلال ٧ أيام من تاريخ الاستلام بشرط أن يكون بحالته الأصلية. سنقوم باسترداد المبلغ كاملاً خلال ٣-٥ أيام عمل من استلام المنتج المراد إرجاعه.</p>
        </div>
        ...
        <div className="glass-card p-6 rounded-2xl mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="font-display text-lg font-bold text-foreground">مدة الاسترداد</h2>
          </div>
          <p className="text-xs text-foreground/50 leading-relaxed">يتم رد المبلغ خلال ٣-٥ أيام عمل من تاريخ استلام المنتج المُرجَع. يتم الرد بنفس طريقة الدفع الأصلية (نقدي أو تحويل بنكي).</p>
        </div>

        <div className="glass-card p-6 rounded-2xl mb-8">
          <h2 className="font-display text-lg font-bold text-foreground mb-4">شروط الإرجاع</h2>
          <div className="space-y-3">
            {conditions.map((c, i) => (
              <div key={i} className="flex items-start gap-3">
                <c.icon size={16} className={`flex-shrink-0 mt-0.5 ${c.type === "success" ? "text-green-500" : "text-red-400"}`} />
                <span className="text-xs text-foreground/50 leading-relaxed">{c.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-card p-6 rounded-2xl">
          <h2 className="font-display text-lg font-bold text-foreground mb-4">كيفية الإرجاع</h2>
